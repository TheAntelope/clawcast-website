...
export function StudioEditor({ initial }: Props) {
  ...
  return (
    <div>
      ...
      <p className={styles.formHint}>
        Dry-runs this draft over sample stories (no TTS spend, saves nothing) so
        you can see the section order, de-AI rewrite, and any market hints.
      </p>
      <div className={styles.formActions}>
        <button
          ...
          disabled={previewing}
        >
          {previewing ? "Previewing…" : "Preview this draft"}
        </button>
      </div>
      ...
      {preview && (
        <div className={styles.tableWrap} style={{ padding: "1rem" }}>
          <div className={styles.formHint}>
            {preview.episode_title} · sections: {preview.section_order.join(" → ") || "—"} ·
            {" "}~{preview.duration_seconds ?? "?"}s
          </div>
          {preview.lint_hits.length > 0 && (
            <div className={styles.notice}>
              Residual style tics in {preview.lint_hits.length} segment(s) after rewrite.
            </div>
          )}
